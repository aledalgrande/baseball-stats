...
RSpec.describe Player, :type => :model do
  context "#import" do
    ...
    context "when there is no player" do
      let(:filename) { "#{Rails.root}/spec/fixtures/players.csv" }
      ...
      it "should import the player id" do
        subject
        expect(Player.first.player_id).to eq('aaronha01')
      end
    end
    ...
    context "when there are players already" do
      ...
      before do
        Player.create(player_id: 'aaronha01', year_of_birth: 1922, first_name: 'Hank', last_name: 'Aaron')
      end

      it "should update the player data" do
        subject
        expect(Player.where(player_id: 'aaronha01').first.year_of_birth).to eq(1934)
      end
    end

    context "when the file is empty" do
      let(:filename) { "#{Rails.root}/spec/fixtures/players2.csv" }
    ...
    context "when the file is corrupted" do
      let(:filename) { "#{Rails.root}/spec/fixtures/players3.csv" }
